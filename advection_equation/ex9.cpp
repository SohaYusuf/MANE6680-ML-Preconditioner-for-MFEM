...
public:
        ...
        dt(-1.0)
   {
      ...
      linear_solver.SetMaxIter(10000);
...
int main(int argc, char *argv[])
{
   ...
   //// added by soha
   int rows = k.Size();
   //int cols = k.GetRowSize(0);

   //cout << "Number of columns in the first row: " << cols << std::endl;

   cout << "Shape of matrix: (" << rows << ")" << endl;

   ofstream writeK("K_ex9.txt");
   k.PrintMatlab(writeK);
   writeK.close();

   ofstream writeM("M_ex9.txt");
   m.PrintMatlab(writeM);
   writeM.close();
   //// added by soha

   

   //ofstream write("b_ex9.txt");
   //b.PrintMatlab(write);
   //write.close();
   ...
   ofstream osol("ex9-final.vtk");
   osol.precision(precision);
   mesh.PrintVTK(osol, 0);
   u.SaveVTK(osol, "Solution", 0);
   osol.close();
   /////////////////////////////////////
   ...
   : TimeDependentOperator(M_.Height()), M(M_), K(K_), b(b_), z(M_.Height())
{
   ...
   if (M.GetAssemblyLevel() == AssemblyLevel::LEGACY)
   {
      ...
      dg_solver = new DG_Solver(M.SpMat(), K.SpMat(), *M.FESpace());  /// by Prof. Hicken
   }
   else
   {
