...
class Model(snt.AbstractModule):
  ...
  def __init__(self, learned_model, name='Model'):
    ...
    with self._enter_variable_scope():
      ...
      self._output_normalizer = normalization.Normalizer(
          size=1, name='output_normalizer')
      self._node_normalizer = normalization.Normalizer(
          size=1+common.NodeType.SIZE, name='node_normalizer')
  ...
  def _build_graph(self, inputs, is_training):
    ...
    node_features = tf.concat([inputs['temperature'], node_type], axis=-1)
  ...
  def loss(self, inputs):
    """L2 loss on temperature."""
    graph = self._build_graph(inputs, is_training=True)
    network_output = self._learned_model(graph)

    # build target temperature change
    cur_temperature = inputs['temperature']
    target_temperature = inputs['target|temperature']
    target_temperature_change = target_temperature - cur_temperature
    target_normalized = self._output_normalizer(target_temperature_change)
  ...
  def _update(self, inputs, per_node_network_output):
    ...
    temperature_update = self._output_normalizer.inverse(per_node_network_output)
    # integrate forward
    cur_temperature = inputs['temperature']
    return cur_temperature + temperature_update