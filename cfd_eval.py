...
def _rollout(model, initial_state, num_steps):
  ...
  def step_fn(step, temperature, trajectory):
    print('')
    ...
    next_temperature = tf.where(mask, prediction, temperature)
    trajectory = trajectory.write(step, temperature)
    return step+1, next_temperature, trajectory

  _, _, output = tf.while_loop(
      cond=lambda step, cur, traj: tf.less(step, num_steps),
      body=step_fn,
      loop_vars=(0, initial_state['temperature'],
